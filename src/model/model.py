...
def preprocessing(data):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        reviews, encoded_labels, test_size=0.2)
...
def training(data):
    ...
    with tf.device("/gpu:0"):
        history = model.fit(X_train_padded, y_train, epochs=1, verbose=1, steps_per_epoch=len(
            X_train)/512, batch_size=32, validation_split=0.25)
